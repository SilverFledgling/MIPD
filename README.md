...
cd "D:\MIPD project\MIPD"

# 2. Khởi tạo git (nếu chưa)
git init

# 3. Tạo repo trên GitHub (vào github.com → New Repository → đặt tên "MIPD")
# Không chọn "Initialize with README" vì đã có sẵn!

# 4. Kết nối remote
git remote add origin https://github.com/<your-username>/MIPD.git

# 5. Add tất cả files
git add .

# 6. Commit
git commit -m "Initial commit: MIPD v1.0 - Model-Informed Precision Dosing"
...
- **Không push `.env`** — file `.gitignore` đã loại trừ
- **Không push `node_modules/`** — đã có trong `.gitignore`
- **Ảnh screenshots**: Đã có trong `docs/screenshots/` (Ảnh1-5.png) ✅
